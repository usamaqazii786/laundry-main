...
class Security_Tweak extends Component {
	...
	public function show_hosting_notice( $option ) {
		...
		/* translators: %s: Option name. */
			__( 'Some hostings do not allow you to make changes to the wp-config.php file. Please contact your hosting support team to switch %s ON or OFF on your site.', 'defender-security' ),
			$option
		);
	}
	...
	public function show_hosting_notice_with_code( $option, $code ) {
		...
		/* translators: %s - option */
			__( "Couldn't change the %s in your wp-config.php file. Please change it manually:", 'defender-security' ) . '<p><b>' . $code . '</b></p>',
			'<b>' . $option . '</b>'
		);
	}
}

	...
	public function get_error_by_file_path( $file_path ) {
		return new WP_Error(
			...
			/* translators: %s - file path */
				__( 'The file %s is not writable', 'defender-security' ),
				$file_path
			)
		);
	}
		...
		if ( 'prevent-php-executed' === $this->type ) {
			if ( ! in_array( Server::get_current_server(), array( 'apache', 'litespeed' ), true ) ) {
				return new WP_Error(
					...
					__( 'The rules can\'t be applied. This can be either because your host doesn\'t allow editing the file, or you\'ve selected the wrong server type.', 'defender-security' )
				);
			}

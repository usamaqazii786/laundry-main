...
class WP_Version extends Component {
	...
	public function to_array() {
		return array(
			'slug'             => $this->slug,
			'title'            => __( 'Update WordPress to latest version', 'defender-security' ),
			/* translators: %s: WP Version */
			'errorReason'      => sprintf( __( 'Your current WordPress version is out of date, which means you could be missing out on the latest security patches in v%s', 'defender-security' ), $this->get_latest_version() ),
			'successReason'    => __( 'You are using the latest version of WordPress, great job!', 'defender-security' ),
			'misc'             => array(
				'latest_wp'       => $this->get_latest_version(),
				'core_update_url' => network_admin_url( 'update-core.php' ),
			),
			'bulk_description' => __( 'Your current WordPress version is out of date, which means you could be missing out on the latest update. We will upgrade WordPress version to the latest.', 'defender-security' ),
			'bulk_title'       => __( 'WordPress Version', 'defender-security' ),
		);
	}
}

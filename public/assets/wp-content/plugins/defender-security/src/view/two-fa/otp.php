...
<head>
	...
	if ( is_multisite() ) {
		...
		$login_header_title = __( 'Powered by WordPress', 'defender-security' );
	}
...
END;
	...
	if ( ! empty( $providers ) ) {
		...
		if ( (is_array($providers) || $providers instanceof \Countable ? count( $providers ) : 0) > 1 ) { ?>
			<div id="wrap-nav">
				<p><?php _e('Having problems? Try another way to log in', 'defender-security' ); ?></p>
				<ul id="nav">
	...
	if ( ! $interim_login ): ?>
        <p id="backtoblog"><a href="<?php
			echo esc_url( home_url( '/' ) ); ?>"><?php
				...
				printf( _x( '&larr; Back to %s', 'site', 'defender-security' ), get_bloginfo( 'title', 'display' ) );
				?></a></p>
	<?php
	endif; ?>
